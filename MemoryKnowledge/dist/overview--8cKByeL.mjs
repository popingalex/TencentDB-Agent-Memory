...
//#region src/engines/wiki/ingest-v2/overview.ts
/**
...
function collectBriefs(wikiDir) {
	const out = [];
	const walk = (dir) => {
		let entries;
		try {
		...
		for (const entry of entries) {
			...
			out.push({
				title: typeof frontmatter.title === "string" && frontmatter.title.trim() ? frontmatter.title.trim() : entry.replace(/\.md$/, ""),
...
*/
async function generateOverview(projectPath, llm) {
