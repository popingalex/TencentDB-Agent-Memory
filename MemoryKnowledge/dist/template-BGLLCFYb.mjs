import { n as parseFrontmatter } from "./frontmatter-Ce2kZn6q.mjs";
import { existsSync, readFileSync } from "node:fs";
import { join } from "node:path";
//#region src/engines/wiki/ingest-v2/template.ts
/**
...
const DEFAULT_PURPOSE = `This knowledge base accumulates and organizes engineering knowledge of software systems,
...
const DEFAULT_SCHEMA = `# Page types
...
function hasMeaningfulContent(body) {
	return body.replace(/^#.*$/gm, "").replace(/Define\b[^.。]*[.。]?/gi, "").replace(/\s+/g, "").length >= 8;
}
function readTemplateFile(projectPath, name) {
	const full = join(projectPath, "wiki", name);
	if (!existsSync(full)) return null;
	try {
		const { body } = parseFrontmatter(readFileSync(full, "utf-8"));
...
*/
function loadTemplate(projectPath) {
...
//#endregion
export { DEFAULT_SCHEMA as n, loadTemplate as r, DEFAULT_PURPOSE as t };
