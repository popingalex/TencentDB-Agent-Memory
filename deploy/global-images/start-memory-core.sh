...
# 静态 memory system user（auth/verify 优先命中，绕过 DB/team/多实例 bug）。
# 每次启动复用固定文件 .system-key（没有则生成），保证用户永远能取到有效 key。
SYS_KEY_FILE="$SCRIPT_DIR/.system-key"
if [[ -s "$SYS_KEY_FILE" ]]; then
  SYS_USER_KEY=$(cat "$SYS_KEY_FILE")
else
  SYS_USER_KEY="sk-mem-$(openssl rand -base64 48 | LC_ALL=C tr -dc 'A-Za-z0-9' | head -c 32)"
fi
...
$DOCKER run -d --name "$CONTAINER" \
  ...
  -e TDAI_INSTANCE_ID="eh" \
  -e TDAI_MEMORY_SYSTEM_USER_ID="usr-sys-ehdev" \
  -e TDAI_MEMORY_SYSTEM_USER_NAME="memory" \
  -e TDAI_MEMORY_SYSTEM_USER_KEY="$SYS_USER_KEY" \
...
# ── 每次启动：从 core metadata 库读出当前有效 admin key，写固定文件 ──
# 目的：保证 .admin-key 里永远是 core 实际认的有效 key（Panel/user_key 从该文件取）。
# core 的 auth/verify 认 default 实例 → 从 default metadata 库读 active system_admin key。
DEFAULT_META_DB="${MEMORY_CORE_VOLUME}/metadata/tdai_metadata_default/metadata.db"
if [[ -f "$DEFAULT_META_DB" ]] && command -v sqlite3 >/dev/null 2>&1; then
  effective_key=$(sqlite3 "$DEFAULT_META_DB" "SELECT k.key_value FROM meta_user_keys k JOIN meta_users u ON u.user_id=k.user_id WHERE u.user_type='system_admin' AND k.status='active' ORDER BY k.created_at LIMIT 1;" 2>/dev/null || true)
  if [[ -n "$effective_key" ]]; then
    # 校验读出的 key 确有效（直达 core default 实例），通过才写文件
    if verify_user_key "$effective_key"; then
      umask 077
      printf '%s' "$effective_key" > "${ADMIN_KEY_FILE}"
      ok "有效 admin key 已写入 ${ADMIN_KEY_FILE}（启动自动持久化）"
    else
      warn "从库读出的 key 校验未过，保留现有 ${ADMIN_KEY_FILE}"
    fi
  else
    warn "default metadata 库未找到 active system_admin key，保留现有 ${ADMIN_KEY_FILE}"
  fi
else
  warn "sqlite3 或 $DEFAULT_META_DB 不可用，跳过自动持久化 key（保留现有 ${ADMIN_KEY_FILE}）"
fi

# ── 静态 system user key 持久化到 .system-key（auth/verify 有效） ──
umask 077
printf '%s' "$SYS_USER_KEY" > "$SYS_KEY_FILE"
ok "静态 system user key（auth/verify 有效）已写入 $SYS_KEY_FILE"
